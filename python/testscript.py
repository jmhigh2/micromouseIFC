...
def graph(mouse):
    ...
    plot1.scatter(position[0]+.5, position[1]+.5, s=100)
    plot1.plot(x_coord, y_coord) #plot optimal path
    ...
    plot2.scatter(position[0]+.5, position[1]+.5, s=100) #position
...
if __name__ == '__main__':
    ...
    for a in range(1,2):
        ...
        mouse.search() #first search. Back at start
        print mouse.cur_pos
        mouse.goback()
        print mouse.cur_pos
        graph(mouse)
    #mouse.search() #second search. back at start again.
